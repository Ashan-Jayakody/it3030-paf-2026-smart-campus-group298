    ...
    <div className="max-w-7xl mx-auto px-6 py-8">
      ...
      {!canManage && (
        <div className="mb-6 rounded-2xl border border-blue-200 bg-blue-50 px-5 py-4 text-sm text-blue-800">
          You are signed in with a read-only role. Resource editing is available to ADMIN and MANAGER.
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl px-5 py-4 flex gap-3 items-center flex-wrap mb-6 shadow-sm border border-gray-100">
        <input
          className="flex-1 min-w-50 px-4 py-2.5 rounded-xl border-2 border-gray-200 text-sm outline-none focus:border-violet-400 transition-colors"
        ...
        {canManage && (
          <button
            onClick={openAdd}
            className="px-5 py-2.5 rounded-xl bg-linear-to-r from-violet-500 to-purple-600 text-white text-sm font-bold hover:opacity-90 transition-opacity shadow-md shadow-violet-200 whitespace-nowrap"
          >
            + Add Resource
          </button>
        )}
      ...
      {!loading && !error && filtered.length === 0 && (
        <div className="bg-white rounded-2xl text-center py-20 border border-gray-100 shadow-sm">
          ...
          {resources.length === 0 && canManage && (
            <button
              onClick={openAdd}
              className="mt-6 px-6 py-3 rounded-xl bg-linear-to-r from-violet-500 to-purple-600 text-white font-bold text-sm hover:opacity-90 transition-opacity"
            >
              Add First Resource
            </button>
          )}
